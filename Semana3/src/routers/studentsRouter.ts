...
router.get("/read/:id", (req: Request, res: Response) => {
  const student = products.find((products) => products.id === req.params.id);

  if (!student) {
    return res.status(400).send({ message: "Nenhum estudante encontrado" });
  }
...
router.post("/store", (req: Request, res: Response) => {
  ...
  if (req.body.age < 18) {
    return res
      .status(401)
      .send({ message: "Estudante deve ser maior de 18 anos." });
  }
  products.push(req.body);
  return res.status(201).send({ message: "Estudante criado" });
});

router.delete("/remove/:id", (req: Request, res: Response) => {
  const studentIndex = products.findIndex(
  ...
  return res.status(200).send({ message: "Estudante removido." });
});
...
router.put("/update/:id", (req: Request, res: Response) => {
  ...
  products[studentIndex] = req.body;
  return res.status(200).send({ message: "Estudante atulizado com sucesso." });
});
